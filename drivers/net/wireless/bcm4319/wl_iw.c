...
extern void msmsdcc_wifi_host_on(void);
extern void msmsdcc_wifi_host_off(void);
...
wl_control_wl_start(struct net_device *dev)
{
	...
	if (g_onoff == G_WLAN_SET_OFF) {
		...
		printk("%s: call msmsdcc_wifi_host_on().\n", __FUNCTION__);
...
)
{
	...
	if (g_onoff == G_WLAN_SET_ON) {
		...
		printk("%s: call msmsdcc_wifi_host_off().\n", __FUNCTION__);
...
static int set_ap_cfg(struct net_device *dev, struct ap_profile *ap)
{
	...
	g_bSoftapRunning = true;
	printk("softap is running, g_bSoftapRunning: %d\n", g_bSoftapRunning);
	...
	if ((wrqu->data.length > 4) && (wrqu->data.length < sizeof(extra))) {
		...
		if  (strstr(fwstr, "apsta") != NULL) {
			  ...
			  if((check_chip & 0xffff) == 0x4319) 
				fwstr = apsta_4319_path;
			  else if((check_chip & 0xffff) == 0x4329)
				fwstr = apsta_4329_path;
